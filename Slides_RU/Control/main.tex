...
\begin{frame}{Линейное управление}
	...
	\begin{flushleft}
		...
		Матрицу $\bo{K}$ мы называем \emph{коэффициентом усиления (gain)}. При таком управлении система примет вид:
...
\begin{frame}{Линейное управление}
	\begin{flushleft}
		...
		\begin{block}{Матрица Гурвица}
			Если матрица $\bo{M}$ имеет собственные значения со строго отрицательными действительными частями, она называется матрицей Гурвица. Мы будем обозначать это как $\bo{M} \in \mathcal{H}$.
		\end{block}
		
		%\bigskip
		
		Таким образом, поиск управления сводится к выбору такого $\bo{K}$, чтобы $(\bo{A} - \bo{B}\bo{K})$ была матрицей Гурвица. 
...
\begin{frame}{Многомерный случай}
	\begin{flushleft}
		...
		Собственные значения замкнутой системы — это $a_{11}-b k_1$ и $a_{22}$. Второе собственное значение не может быть изменено выбором коэффициентов управления. Если $a_{22} < 0$, нам достаточно выбрать $k_1$ так, чтобы $a_{11}-b k_1 = q$, где $q$ — отрицательное: $k_1 = \frac{ a_{11} - q}{b}$.
...
\begin{frame}{Пружина-масса-демпфер, 2}
	...
	\begin{flushleft}
		...
		Проанализируем собственные значения $\lambda$ для случая если \textcolor{mydarkgreen}{$\mu > 0$} и \textcolor{mydarkgreen}{$ c > 0$}:
		%
		\begin{align*}
			\lambda = \frac{-\mu \pm \sqrt{\mu^2 - 4 c} }{2}
		\end{align*}
...
\begin{frame}{Пружина-масса-демпфер, 3}
	\begin{flushleft}
		...
		$$\lambda = \frac{-\mu \pm \sqrt{\mu^2 - 4 c} }{2}$$
		%
		Если \textcolor{red}{$c < 0$}, то $\sqrt{\mu^2 - 4c} \geq \mu$, и собственные значения вещественные, причём одно из них \textcolor{red}{положительное}, система неустойчива (вне зависимости от знака $\mu$).
...
\begin{frame}{Размещение полюсов (Pole-placement)}
	\begin{flushleft}
		...
		\begin{itemize}
			\item В MATLAB это функция \texttt{K = place(A,B,p)}, где \texttt{p} — желаемые собственные значения матрицы \texttt{(A-B*K)}.
			
			\item В Python это функция \texttt{scipy.signal.place\_poles(A, B, poles)}.
		\end{itemize}
...
\begin{frame}{Слежение за траекторией, 3}
	...
	\begin{flushleft}
		...
		\begin{equation}
			\bo{u}^* = \bo{B}^\dagger(\dot{\bo{x}}^*  - \bo{A}\bo{x}^*)
		\end{equation}				
