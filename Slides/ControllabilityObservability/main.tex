...
\begin{frame}{PBH controllability criterion, 1}
	\begin{flushleft}
		
		Consider a system $\dot{\bo{x}}=\bo{A}\bo{x}+\bo{B}\bo{u}$. Let $\bo{v}$ be a left-eigenvector of $\bo{A}$, meaning $\bo{v}\T \bo{A} = \lambda \bo{v}\T$. Then, we can multiply the dynamics by $\bo{v}\T$ (we can \emph{project} it onto $\bo{v}$):
		%
		\begin{align}
		...
		\end{align}
		%
		If $\bo{v}$ is orthogonal to $\bo{B}$, meaning $\bo{v}\T\bo{B} = 0$, then we get:
		%
		\begin{align}
			\bo{v}\T \dot{\bo{x}}=\lambda \bo{v}\T \bo{x}
		\end{align}
		%
		Define $s = \bo{v}\T \bo{x}$ to get an autonomous system:
		%
		\begin{align}
			\dot{s}=\lambda s
		\end{align}
		
		This means there is a direction in the state-space which cannot be controlled. Conversely, if there exists an uncontrollable direction, then it is an invariant subspace and it should contain an least one eigenvector $\bo{v}$.
				
	\end{flushleft}
\end{frame}


\begin{frame}{PBH controllability criterion, 2}
	\begin{flushleft}
		
		This is Popov-Belevitch-Hautus controllability test:
		
		\begin{block}{PBH controllability criterion, 1}
			If there exists such $\bo{v}$ that $\bo{v}\T \bo{A} = \lambda \bo{v}\T$ and $\bo{v}\T\bo{B} = 0$, then the system is not controllable.
		\end{block}
		
		\begin{block}{PBH controllability criterion, 2}
...
\begin{frame}{PBH controllability criterion, 3}
	\begin{flushleft}
		
		Both versions of the PBH test are useful: 
		
		\begin{itemize}
			\item we can use the compute the smallest singular value of the matrix $[ (\bo{A}-\lambda\bo{I}), \bo{B}]$ to find how close a given eigenvalue $\lambda$ gets us to uncontrollability; 
			
			\item then we could study its associated left eigenvector $\bo{v}$ to find which direction in the state-space i snearly (or completely) uncontrollable.
		\end{itemize}
