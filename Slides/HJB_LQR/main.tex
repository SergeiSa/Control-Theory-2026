...
\begin{frame}{Pole placement - issues, 1}
	\begin{flushleft}
		...
		We can find such control law $u = - k_1 x_1- k_2 x_2- k_3 x_3- k_4x_4$ that would make the system stable. However, pole placement would struggle with thios task, because of the eigenvalue -1, which cannot be influenced.
...
\begin{frame}{Pole placement - issues, 1}
	\begin{flushleft}
		...
		In general, pole placement requires you to:
		
		\begin{itemize}
			\item know what eigenvalues to ask for;
			
			\item has trouble working with systems where some some of the eigenvalues cannot be influenced;
			
			\item has trouble with specific tasks, like setting multiple egenvalues to the same value;
			
			\item can lead to unreasonably high gains.
		\end{itemize}
		
		We have a better method, one that discovers "good eigenvalues" based on how accurate and how economical (in terms of control effort) uyou want your system to be - \emph{linear quadratic regulator (LQR)}.
...
\begin{frame}{Continuous-time Dynamic Programming Principle (DPP)}
	\begin{flushleft}
		...
		\end{equation}
		%
		It implies that $J^*(\bo{x}^*(t_2))$, which is "closer to the goal", is smaller than $J^*(\bo{x}^*(t_1))$ by $\Delta J$; where as $\Delta J$ represents the additional cost needed to go from $\bo{x}^*(t_1)$ to $\bo{x}^*(t_2)$. This is the \emph{Continuous-time Dynamic Programming Principle (DPP)}.
