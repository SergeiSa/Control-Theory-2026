...
Consider Autonomous dynamic system $\dot{\bo{x}} = \bo{f}(\bo{x})$ where $ \bo{f}(0) = 0$. Then:

\begin{block}{Asymptotic stability}
This system is proven to be asymptotically stable, if there exists a scalar function $V = V(\bo{x}) > 0$, whose time derivative is negative $\dot V(\bo{x}) < 0$, except $V(\bo{0}) = 0$, $\dot V(\bo{0}) = 0$.
\end{block}
...
\begin{definition}
Function $V(\bo{x}) > 0$ satisfying these conditions is called \emph{Lyapunov function}.
\end{definition}
...
This satisfies the asymptotic stability conditions.
...
\begin{frame}{Lyapunov method: Example 2}
	\begin{flushleft}
		...
		This satisfies the Lyapunov criteria, so the system is stable. It is not proven to be asymptotically stable, because $\dot V(q, \dot{q}) = 0$ for all $(q, \dot q)$ such that $\dot q = 0$.
...
Given $\dot{\bo{x}} = \bo{A}\bo{x}$ and $V = \bo{x}^\top\bo{S}\bo{x} > 0$, let's find its derivative:
...
Notice that $\dot V(x)$ should be negative for all $\bo{x}$ for the system to be asymptotically stable, meaning that $\bo{A}^\top\bo{S} + \bo{S}\bo{A}$ should be negative definite. Another form of this requirement is \emph{Lyapunov equation}:
...
Given $\bo{x}_{i+1} = \bo{f}(\bo{x}_i)$, if $V(\bo{x}_i) > 0$, and $V(\bo{x}_{i+1}) - V(\bo{x}_i) < 0$, the system is asymptotically stable.
\end{block}
...
Consider dynamics $\bo{x}_{i+1} = \bo{A}\bo{x}_i$ and $V = \bo{x}_i^\top\bo{S}\bo{x}_i > 0$, let's find $V(\bo{x}_{i+1}) - V(\bo{x}_i)$:
...
Notice that $V(\bo{x}_{i+1}) - V(\bo{x}_i)$ should be negative for all $\bo{x}_i$ for the system to be asymptotically stable, meaning that $\bo{A}^\top\bo{S}\bo{A} - \bo{S}$ should be negative definite, giving us \emph{Discrete Lyapunov equation}:
