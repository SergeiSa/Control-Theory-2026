...
Then such state $\bo{x}_0$ is called \emph{equilibrium point}, \emph{node} or a \emph{critical point}.
...
Equilibrium point $\bo{x}_0$ is called \emph{stable} (or \emph{Lyapunov stable}) iff for any constant $\varepsilon >0$ there exists constant $\delta >0$ such that:
...
For a stable equilibrium, trajectories will not leave an $\varepsilon$-sized ball if they start form a sufficiently small $\delta$-sized neighbourhood. Pick any value of $\varepsilon$, there always is such $\delta$ that \eqref{eq:stability_def} holds.
...
A Lyapunov stable equilibrium point $\bo{x}_0$ is called \emph{asymptotically stable} iff for some constant $\delta$ it is true that:
...
Think of it as "for any initial point that lies at most $\delta$ away from $\bo{x}_0$, the trajectory $\bo{x}(t)$ will asymptotically approach $\bo{x}_0$".

\bigskip

Equivalently we can say "the solutions starting from $\delta$-sized ball around $\bo{x}_0$ converge to $\bo{x}_0$".
...
\begin{frame}{Stability of an Autonomous LTI}
...
The product $\dot{\bo{x}}^\top \bo{x} < 0$ is negative iff $\alpha < 0$.
...
\begin{definition}
Eq. \eqref{eq:LTI} is asymptotically stable iff the real parts of eigenvalues of $\bo{A}$ are negative. Such matrix $\bo{A}$ is called Hurwitz.
\end{definition}


\begin{definition}
	Eq. \eqref{eq:LTI} is stable iff the real parts of eigenvalues of $\bo{A}$ are non-positive, assuming $\bo{A}$ is diagonilizable over $\mathbb{C}^n$.
\end{definition}
...
\begin{frame}{Appendix A}
	\framesubtitle{Jordan blocks and stability}
	\begin{flushleft}
		
		So far, we considered systems $\dot{\bo{x}} = \bo{A} \bo{x}$ where matrices are diagonalizable over  $\mathbb{C}^n$. We did not consider matrices with non-trivial Jordan blocks. 
		
		\bigskip
		
		Consider a counter-example, matrix $ \bo{A} = \begin{bmatrix}
			0 & 1 \\  0 & 0
		\end{bmatrix}$ with two zero eigenvalues, representing the system:
		...
		\end{equation}
		 ...
		 The solution to this system is: $x_1 (t) = x_2(0) t$ and $x_2(t) = x_2(0) $ - the solution diverges, even though the eigenvalues are zero.
...
\begin{frame}{Appendix A}
	...
	\begin{flushleft}
		...
		We can make the marginal (Lyapunov) stability criterion more precise:

		\begin{definition}
			Eq. \eqref{eq:LTI} is stable iff the real parts of eigenvalues of $\bo{A}$ are non-positive, and none of the zero-real-part eigenvalues are associated with a non-trivial Jordan block.
		\end{definition}
