...
%	\end{frame}
	...
	\begin{frame}{Singular values}
		\begin{flushleft}
			...
			Its transfer function is $G(s) = \bo{C} (s\bo{I} - \bo{A})^{-1} \bo{B}$. The frequency response at $\omega = \omega_0$ is:
			
			\begin{equation}
				G(j\omega_0) = \bo{C} (j\omega_0\bo{I} - \bo{A})^{-1} \bo{B}
			\end{equation}
			
			The spectral norm of the frequency response matrix (its largest singular value) represents the largest input amplification that is possible at the frequency $ \omega_0$:
	...
	\begin{frame}{H-infinity conditions}
		...
		\textcolor{mygray}{(assuming $\bo{x}(0) = 0$).}
	...
	\begin{frame}{Storage function criterion}
		\begin{flushleft}
			...
			The term $\bo{y}\T \bo{y} - \gamma^2 \bo{w}\T \bo{w} $ is called the \emph{supply rate}.
	...
	\begin{frame}{Storage function criterion - proof}
		\begin{flushleft}
			...
			Assume zero initial conditions (we consider pure input-output behaviour), so $V(\bo{x}(0)) = 0$ and remember that $V$ is positive-definite:
	...
	\begin{frame}{Derivation of LMI conditions, 1}
		\begin{flushleft}
			...
			Consider a storage function of the form $V( \bo{x}) =  \bo{x}\T  \bo{P}  \bo{x}$, where $ \bo{P} \succ 0$. Then the criterion becomes:
...
\begin{frame}{H-infinity control design, 2}
	\begin{flushleft}
		...
		Apply the Schur complement to linearize the quadratic term and define $\bo{L} = \bo{K}\bo{Q}$ to get the LMI:
		%
		\begin{align}
			\begin{bmatrix}
...
\end{frame}
	...
	\begin{frame}[fragile]{H-infinity control design: CVXPY example}
		\begin{flushleft}
			{\small
			\begin{verbatim}
import cvxpy as cp
