...
\begin{frame}{Pole-placement}
	\begin{flushleft}
		...
		\begin{itemize}
			\item  In MATLAB there is a function \texttt{K = place(A,B,p)}, where \texttt{p} are the desired eigenvalues of \texttt{(A-B*K)}.
			
			\item In Python there is a function \texttt{scipy.signal.place\_poles(A, B, poles)}.
		\end{itemize}
