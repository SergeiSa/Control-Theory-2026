...
\begin{frame}{Linear systems, static}
	\begin{flushleft}
		...
		For some systems, the relation between input and output is proportional. This is an example of a emph{static} system.
		...
		\begin{definition}
			A system is called static (or memoryless) if the output at time $t$ depends only on the input at time $t$.
		\end{definition}
...
\begin{frame}{Linear systems, dynamic}
	\begin{flushleft}
		...
		Alternatively, there are linear systems for which relation between the input and the output depends on derivatives of these signals. Similarly, there are linear systems where the relation between the input and the output  depends on the internal states. We can call such systems \emph{dynamic}.
...
\begin{frame}{Controller+Observer}
	\begin{flushleft}
		...
		The form of this system resembles a plant with output equation.
...
\begin{frame}{PID, 1}
	\begin{flushleft}
		...
		One of the better known examples of dynamic controlllers is a \emph{proportional-integral-derivative controller (PID)}. In the scalar case:
...
%		\bigskip
		...
		This controller is similar to the linear controllers we studied in the course (e.g. LQR) but with the integral part allowing for \emph{robustness}: the ability to compensate for static additive disturbance in the model.
...
\begin{frame}{PID, 2}
	\begin{flushleft}
		...
		We can re-write the PID in Laplace space:
		...
		It is not instantly obvious that the controller has an internal state.
...
\begin{frame}{Static disturbance, 1}
	\begin{flushleft}
		...
		Even if a static state-feeback delivers assymptotic stability, a constant disturbance leads to a nonzero steady-state error.
...
\begin{frame}{Filters, 2}
	\begin{flushleft}
		...
		Frequency response of this transfer function is:
