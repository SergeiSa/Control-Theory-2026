...
\begin{itemize}
    \item There are no derivatives in the equation; the system is described by a difference equation, not an ODE;
    \item Time evolves in discrete steps $i = 0,1,2,\dots$;
...
\begin{frame}{Stability of the Discrete Dynamics}
	\begin{flushleft}
		...
		With eigendecomposition $\bo{A} = \bo{V} \bo{D} \bo{V}^{-1}$ (where $\bo{D}$ is a diagonal matrix with eigenvalues $\lambda_j$ of $\bo{A}$ on its diagonal)  and introducing notation $\bo{z}_i = \bo{V}^{-1}\bo{x}_i$ we get:
...
\begin{frame}{Stability of the Discrete Dynamics}
	...
	\begin{flushleft}
		...
		Considering individual equations $z_{j, i+1} =  \lambda_j z_{j, i}$ we find that the absolute value of the scalars $z_{j}$ converges to zero iff $|\lambda_j| < 1$:
...
\begin{frame}{Stability of the Discrete Dynamics}
	\framesubtitle{Complex eigenvalue pair}
	\begin{flushleft}
...
\begin{frame}{Stability of the Discrete Dynamics}
	...
	| \lambda |^2
		...
		We can see that $\bo{x}$  converges to zero iff $|\lambda| < 1$.
...
Discrete system $\bo{x}_{i+1} = \bo{A}\bo{x}_i$ is asymptotically stable as long as the eigenvalues of $\bo{A}$ are smaller than 1 by absolute value: $|\lambda_i(\bo{A})| < 1, \; \forall i$.
\end{block}

\begin{block}{Marginal Stability}
Discrete system $\bo{x}_{i+1} = \bo{A}\bo{x}_i$ is marginally stable as long as the eigenvalues $|\lambda_i(\bo{A})| \leq 1, \; \forall i$ \textcolor{mynormalgray}{and no eigenvalue on the unit circle is associated with a non-trivial Jordan block}.
\end{block}
...
\begin{frame}
	...
	\centering{\huge Continuous-time LTI: Analytical solution}
...
\begin{frame}{From analytical solution to discrete dynamics}
	...
	\begin{flushleft}
		...
		Given a solution $\bo{x}(t)$ we can study how the system evolves from $t=0$ to $t=\Delta t$, assuming that $\bo{u}(t) = \bo{u}_0 = \text{const}$ on $\ t \in [ 0, \ \Delta t ]$, and denoting $\bo{x}(0) = \bo{x}_0$ and $\bo{x}(\Delta t) = \bo{x}_1$:
...
{
	The same could be derived by replacing the derivative in $\dot{\bo{x}} = \bo{A}  \bo{x}$ with a finite-difference approximation:}
%
\begin{equation*}
	\textcolor{mynormalgray}
...
Approximating \emph{discrete state space matrix} $\bar{\bo{A}}$ and \emph{discrete control matrix} $\bar{\bo{B}}$ as follows:
...
\begin{frame}{Jordan blocks and stability, 1}
	\begin{flushleft}
		
		Consider a matrix $\bo{A} \in \R^{n \times n}$ with a single Jordan block of size $n$ (the entire matrix is a Jordan block). Any Jordan block can be written as:
		%
		\begin{equation}
...
\begin{frame}{Jordan blocks and stability, 2}
	\begin{flushleft}
		...
		Analysing the solution we notice:
		
		\begin{itemize}
			\item If $\lambda < 0$: exponential decay dominates, the system is stable;
			\item If $\lambda = 0$: polynomial growth, the system is unstable;
			\item If $\lambda > 0$: exponential growth, unstable.
		\end{itemize}
