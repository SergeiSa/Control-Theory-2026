...
\begin{frame}{Inner and outer products}
	\begin{flushleft}
		
		Inner product $\bo{x}\T \bo{x}$ and outer product $\bo{x}\bo{x}\T$ are defined as:
		%
		\begin{align*}
			\bo{x} &= 
			\begin{bmatrix}
...
A \emph{random variable} $\bo{v}$ produces values according to a probability distribution. We can observe realizations $\bo{v}_1$, $\bo{v}_2$, $\bo{v}_3$, ... sampled from the distribution.
...
\begin{frame}{Mean of a linear transform}
	\begin{flushleft}
		...
		Let $\bo{x}$ be a random variable $\bo{x} \sim \mathcal{N} (\bar{\bo{x}}, \bo{X})$.  Given a constant matrix $\bo{M}$ we can define an linear transformation of $\bo{x}$:
...
\begin{frame}{State estimation error - covariance}
	\begin{flushleft}
		...
		Assume that $\bo{w}$ is independent of the state (and estimation error), meaning that $E[\btil{x}_i \bo{w}_i\T] = E[\bo{w}_i \btil{x}_i\T] = 0$:
